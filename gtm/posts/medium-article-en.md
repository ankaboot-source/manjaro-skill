...
You ask it to install `htop`. It suggests `pip install htop`.

You ask it to install `prettier`. It suggests `npm install -g prettier`.
...
Now when I ask my AI to install `httpie` on Manjaro:

```
Before: sudo pip install httpie    # 😱
After:  sudo pacman -S httpie     # 😎
```

Or if not in the repos:
